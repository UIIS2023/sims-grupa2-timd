...
{
    public class AccommodationReservationRescheduleCsvRepository : IAccommodationReservationRescheduleRepository
    {
        ...
        public AccommodationReservationRescheduleCsvRepository()
        {
        ...
        {
            _accommodationReservationReschedules = _serializer.FromCsv(FilePath);
            return _accommodationReservationReschedules.FindAll(r => r.Reservation.Id == reservation.Id);
        }

        public bool NewNotificationExists(User user)
        {
            _accommodationReservationReschedules = _serializer.FromCsv(FilePath);
            return _accommodationReservationReschedules.Any(r => r.Guest.Id==user.Id && r.RequestStatus != Status.Waiting && r.NotifyUserOfStatusChange);
        }
    }
}
