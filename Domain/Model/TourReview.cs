...
namespace SimsProject.Domain.Model
{
    public class TourReview : ISerializable
    {
        ...
        public string[] ToCsv()
        {
            string[] csvValues = { Id.ToString(), GuideKnowledge.ToString(), GuideLanguage.ToString(), TourInterestigness.ToString(), Comment, Guest.Id.ToString(), Reservation.Id.ToString(), TourDate.Id.ToString(), Tour.Id.ToString(), CheckPoint.Id.ToString(), IsValid.ToString(), Cover.Id.ToString() };
            return csvValues;
        }
        ...
        public void FromCsv(string[] values)
        {
            ...
            Tour = new Tour() { Id = Convert.ToInt32(values[8]) };
            CheckPoint = new CheckPoint() { Id = Convert.ToInt32(values[9]) };
            IsValid = bool.Parse(values[10]);
            Cover = new Image() { Id = Convert.ToInt32(values[11]) };
        }
    }
}
